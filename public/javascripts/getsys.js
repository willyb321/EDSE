	...
	return new Promise((resolve, reject) => {
		...
		.then(resdata => {
			if (resdata[0] !== 'N/A' && resdata[2]) {
				const distance = distanceGet(parseInt(resdata[2][0].x), parseInt(resdata[2][0].y), parseInt(resdata[2][0].z), parseInt(resdata[2][1].x), parseInt(resdata[2][1].y), parseInt(resdata[2][1].z)).toFixed(2);
